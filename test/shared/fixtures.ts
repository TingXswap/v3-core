...
async function factoryFixture(): Promise<FactoryFixture> {
  ...
  const factory = (await factoryFactory.deploy()) as any
  return { factory }
}
...
async function tokensFixture(): Promise<TokensFixture> {
  ...
  const tokenA = (await tokenFactory.deploy(BigNumber.from(2).pow(255))) as any
  const tokenB = (await tokenFactory.deploy(BigNumber.from(2).pow(255))) as any
  const tokenC = (await tokenFactory.deploy(BigNumber.from(2).pow(255))) as any
...
export const poolFixture: Fixture<PoolFixture> = async function (): Promise<PoolFixture> {
  ...
  const swapTargetCallee = (await calleeContractFactory.deploy()) as any
  const swapTargetRouter = (await routerContractFactory.deploy()) as any
  ...
  return {
    ...
    createPool: async (fee, tickSpacing, firstToken = token0, secondToken = token1) => {
      const mockTimePoolDeployer = (await MockTimeTingswapV3PoolDeployerFactory.deploy()) as any
      ...
      return MockTimeTingswapV3PoolFactory.attach(poolAddress) as any
    },
  }
}
